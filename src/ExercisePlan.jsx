...
const ExercisePlan = () => {
  ...
  useEffect(() => {
    ...
    if (cookieData) {
      ...
      setBmi(parsedData.bmi);  // Set bmi here first
  
      if (parsedData.bmi) {  // Only call recommendationLogic when bmi is available
        const plan = recommendationLogic(parsedData.bmi);  // Pass bmi directly
